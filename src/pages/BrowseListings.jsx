            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {filteredListings.length > 0 ? (
                        ...
                        <div className="col-span-3 bg-white border rounded-lg p-10 text-center">
                            <h2 className="text-lg font-semibold text-gray-800">No listings found</h2>
                            <p className="text-gray-500 mt-2">
                                {listings.length === 0
                                    ? 'No service providers have published a listing yet.'
                                    : 'Try clearing the search or choosing a different category.'}
                            </p>
                            {listings.length === 0 && user?.role === 'provider' && (
                                <button
                                    onClick={() => navigate('/dashboard/new-listing')}
                                    className="mt-5 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700"
                                >
                                    Create a Listing
                                </button>
                            )}
                            {listings.length === 0 && !user && (
                                <Link to="/register" className="inline-block mt-5 text-blue-600 hover:underline">
                                    Register as a provider to add the first listing
                                </Link>
                            )}
                        </div>
                    )}
